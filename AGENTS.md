...
## Governança para SaaS Multi-empresa

Estas regras aplicam-se a qualquer projeto SaaS hospedado neste repositório:

- **Isolamento de Dados (Tenancy)**: Toda consulta (Query) deve obrigatoriamente incluir o filtro por empresa (`company_id`). Utilize Traits como `BelongsToCompany` e Global Scopes para automatizar isso. **Nunca** gerar consultas globais sem verificação explícita de permissão do usuário de suporte central.
- **Configurações Dinâmicas**: Não codificar limites ou planos no código. Utilizar tabelas de configuração ou pacotes de "Subscriptions" para validar acessos e funcionalidades permitidas por empresa.
- **Segurança de Assets**: Mídias enviadas por chat ou upload devem ser armazenadas em pastas isoladas por empresa no Storage, garantindo que o URL de uma empresa não seja previsível por outra.

---

## Governança para Atendimento Omnichannel e Chatbot

Diretrizes para o módulo de chat e automação:

- **Estado da Conversa**: Utilizar drivers de cache (Redis) ou tabelas de sessão para gerir o estado do fluxo do Chatbot, evitando consultas pesadas ao banco em cada mensagem recebida.
- **Logs de Auditoria de Chat**: Toda ação de "Espiar Conversa" (Spy Mode) ou alteração manual de ticket deve gerar um log de auditoria vinculando o ID do atendente, timestamp e contexto.
- **Estabilidade de Integração**: Tratamento rigoroso de exceções em webhooks e chamadas de API externas (WhatsApp). Sempre utilizar Queues (Jobs) para processamento de mensagens recebidas para não bloquear o servidor web.
- **Privacidade (LGPD)**: Implementar mascaramento de dados sensíveis em logs técnicos. Garantir que o usuário final possa solicitar a exclusão de seu histórico via fluxo automático ou manual.
